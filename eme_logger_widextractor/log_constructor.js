...
// Custom
function makeOnBothDigit(n){
    return Number.parseInt(n, 16) > 0xf ? "" + n: "0" + n;
}
...
emeLogConstructor.buildObjectItem = function(data, indent) {
	...
	return data.toString(16); // vanilla: toString();
        ...
        while (data.values.length > 0) {
            ...
            for (var i = 0; i < tmp.length; i++) {
                if (i >= old_len)
                    ...
                    num = Number.parseInt(tmp[i]);
                    convertedData +=  makeOnBothDigit(num.toString(16)) + " ";
                }
            }
            convertedData += "| ";
            for (var i = 0; i < tmp.length; i++) {
